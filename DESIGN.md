...
> **Open — the bottom strip holds two objects, and we will want three.** Measured on iOS 26, not recalled: Messages (no tab bar) floats a search capsule at the bottom with compose in the top-right; Photos (tab bar) puts the `[Library | Collections]` capsule bottom-left and search as a detached circle bottom-right. Both are a capsule on the left and a circle on the right — the shape Ponor already has for search and `+`. Neither puts search at the top. When the tab bar arrives (with Settings, the first screen after M1d), tabs claim the left capsule and the strip is full, so search or `+` must move. Apple avoids the question by never pairing a tab bar with a create button; §3 and §0.5 do not let us. **Leaning:** `[tab capsule] [+ circle]`, with search becoming a pull-to-reveal field on the Dives screen — the `+` keeps the thumb slot §0.5 reserves for it, and a field hidden until you pull down does not reproduce the "search attracts the most attention" problem that moved it off the top in the first place. Decide it when Settings lands, with a screenshot, not now.
