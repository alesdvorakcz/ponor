...
**Web is pulled forward out of v1.1** (owner's call, during M1d) — but as a **testing target, not a supported platform**: no store listing, no parity promise, and no obligation on later features to work there. The reason it moves is not only that a browser is a faster way to actually use the app than a device build; it is that the blocker lives in the **data layer**, which M2's sync will be built on top of. Finding out at v1.1 that synchronous SQLite-over-a-worker cannot work on web would be expensive; finding out now is cheap. Diagnosed rather than assumed: the build only failed because Metro would not resolve a `.wasm` specifier (one line, fixed); `wa-sqlite` then needs cross-origin isolation headers from the host; and the remaining "Sync operation timeout" is a **race, not a misconfiguration** — expo-sqlite's bridge spins a budget counted in iterations (`1e6` × `Atomics.pause()`, a few milliseconds) while the worker is still compiling 621 KB of wasm, and `db/client.ts` makes its first `openDatabaseSync` call at module load. The expected fix is to warm the worker asynchronously at startup, not to change the data layer. Two known costs: `@react-native-community/datetimepicker` ships no web build, so the date and time fields need a `.web` variant (an HTML `<input type="date">` keeps §10's "an invalid date cannot be entered" intact), and `react-native-maps` has no web support, so M2's Map tab will not render there.
