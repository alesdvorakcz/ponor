...
**Search and `+` share one capsule at the top right.** It began at the bottom — measured off iOS 26 Messages, and correct while Ponor had no tab bar — and moved when the tabs arrived and claimed that strip (§3). The capsule follows Calendar: one rounded glass container, magnifier and `+` inside it as **equal monochrome glyphs**, no border and no rule, resting on the content with the list running full width underneath. Liquid Glass where the device has it (`expo-glass-effect`, guarded by `isLiquidGlassAvailable()`), a plain `surface` capsule everywhere else — which is the common case and must look deliberate rather than degraded. The `+` gets **no extra emphasis** over the magnifier: §10 already rules out a hue, and size or weight would reintroduce the "is this data or chrome" question §0.1 exists to prevent.
...
**The capsule recedes as the list scrolls down and returns on the way up.** This is not decoration, and it was briefly dropped in the move to the top right: a trip header's date range sits in its **trailing slot**, exactly where the capsule floats, so every sticky header that scrolled past had its date occluded — observed clipped to `…16`. The recede also earns its keep the original way, since a logbook is scanned far more often than searched. One accepted residue: while the capsule *is* showing, it still covers a sticky header's date. Removing that entirely would mean reserving capsule-width padding in every trip header forever, or hiding the capsule somewhere it cannot be reached — both larger than the problem.
