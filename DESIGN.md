...
**The Dives screen is a native large-title screen.** Measured off iOS 26 rather than recalled, and it took three shapes to get here. The capsule sits in a **pinned bar** whose top clearance is the **device's safe-area inset**, not a number — 62 pt on an iPhone 17 Pro, which is exactly where Files puts its own trailing `•••`. The large **"Dives"** title lives *below* it in the scroll content and scrolls away, as Files' "Shared" does. **No compact title fades in when it goes**: the tab bar already shows which screen is selected, so the screen is never unidentified, and a scroll-driven crossfade is machinery with an edge case at every rubber-band.
