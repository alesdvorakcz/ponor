...
**Every top-level screen carries a title, and the capsule sits in that row.** "Dives" and "Settings" lead their screens; on Dives the capsule sits at the row's trailing edge, on one line with the title. This is what finally fixed a defect the capsule went through two shapes to reach: floating over the list, it covered the **trailing slot of every sticky trip header**, which is where a trip's date range lives — observed clipped to `…16`. The first answer was to make the capsule recede on scroll, which worked and cost something quietly: it hid the `+` exactly when a diver deep in a long logbook wanted it, so logging a dive meant scrolling to the top first. With the row in flow the list's viewport simply begins beneath it, so no sticky header can reach the capsule at any offset — the occlusion is structurally impossible rather than avoided, and the capsule never leaves. `useHideOnScroll` existed only for the recede and was deleted with it.
